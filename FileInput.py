...
class BrowseWindow(QtWidgets.QMainWindow):
       ...
       try:
        if inputFile.endswith('.json'):
            ...
            if(metrics.iloc[:,0].count()<2) :
                QMessageBox.about(UI_MainWindow.Ui_MainWindow.tab,"Error:" ,"There are not enough samples in your file to conduct analysis. Please choose another file.")
                UI_MainWindow.Ui_MainWindow.onBrowseClicked(UI_MainWindow.Ui_MainWindow)
            return metrics
    ...
    def FileCheck(path):       
       try:
           ...
           QMessageBox.about(UI_MainWindow.Ui_MainWindow,"Message from Assurance: " ,"Error loading file...")
           return 0 

    def SpectralCountsFileMatchNames(self,spectralCounts):
        if(UI_MainWindow.Ui_MainWindow.metrics.iloc[:,0]!= spectralCounts.iloc[:,0]):
            QMessageBox.about(UI_MainWindow.Ui_MainWindow.tab,"Error:" ,"The column first column of the spectral counts file does not match that of the quality metrics input file. Try again.")
            UI_MainWindow.Ui_MainWindow.onBrowseClicked(UI_MainWindow.Ui_MainWindow)
    ...
    def SpectralCountsfiletypeCheck(inputFile):
        if inputFile.endswith('.csv'):
            ...
            BrowseWindow.SpectralCountsFileMatchNames(BrowseWindow,spectralCounts)
