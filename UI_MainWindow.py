...
class Ui_MainWindow(QtWidgets.QTabWidget):
    ...
    def onOutliersClicked(self):
        ...
        sampleToVariableRatio = PCA.PCA.calculateSampleToVariableRatio(self, Ui_MainWindow.NumericMetrics)
        if(sampleToVariableRatio<5):
            if(len(Ui_MainWindow.NumericMetrics.iloc[:,0])< 100):
                QMessageBox.about(self,"Warning:" ,"Consider consulting PCA literature to ascertain whether the ratio of sample size to number of variables is sufficient to perform PCA in this dataset.")
        ...
        Ui_MainWindow.PCA.Checkbox.stateChanged.connect(lambda x: Ui_MainWindow.enable_slot() if x else Ui_MainWindow.disable_slot())
        ...
        #gridlayout.addWidget(Ui_MainWindow.PCA.plotlabel,zip(0,0))
    ...
    def onLongitudinalClicked(self):
        ...
        #InputtingFile:
        QMessageBox.about(self,"You have selected Longitudinal analysis." ,"You will be asked to select a separated value file (.tsv or .csv) containing two columns. The first should contain the filename and the second the spectral counts. From the corresponding graph you will select which samples are to be used for the guide set.")
        FileInput.BrowseWindow.__init__(FileInput.BrowseWindow,Ui_MainWindow)
        spectralCountsFile = FileInput.BrowseWindow.GetSpectralCountsFile(Ui_MainWindow)
    ...
    def update_annot(event):
        ...
        closestx = np.unravel_index((np.abs(PCA.plotdata - event.xdata)).argmin(), PCA.plotdata.shape)
        PCAGraph.annot.xyann = (PCA.plotdata[closestx[0],0], PCA.plotdata[closestx[0],1])
       # annot.set_xy(PCAGraph.PCAGraph.plotdata[closestx[0],0], PCAGraph.PCAGraph.plotdata[closestx[0],1])
        samplenames = DataPreparation.DataPreparation.FindRealSampleNames(Ui_MainWindow, Ui_MainWindow.metrics.iloc[:,0])
        if(len(samplenames)!=len(set(samplenames))): #if there are duplicates in the filenames column like RTsegments or per swath metrics
            sampleNameColumn1Combination = samplenames[closestx[0]]+"-"+str(Ui_MainWindow.metrics.iloc[closestx[0],1])
            text = sampleNameColumn1Combination.format( PCA.plotdata[closestx[0],0], PCA.plotdata[closestx[0],1] )
        else:
            text = samplenames[closestx[0]].format( PCA.plotdata[closestx[0],0], PCA.plotdata[closestx[0],1] )
        
        PCAGraph.annot.set_text(text)    

    def CreateRandomForestTab(self):
        #Create the tab which will contain the graph:
        spectralCountsPlot = IndividualMetrics.MyIndMetricsCanvas(Ui_MainWindow.spectralCountTable,Ui_MainWindow.spectralCountTable, 1, True) # Here element refers to the column index used for the y-value
        ...
        hbox3.addStretch()
        Ui_MainWindow.spectralCounts.goodbtn = QtWidgets.QPushButton('This is my selection for desired spectral counts.', Ui_MainWindow.spectralCounts)
        Ui_MainWindow.spectralCounts.goodbtn.setEnabled(False)
        Ui_MainWindow.spectralCounts.badbtn = QtWidgets.QPushButton('This is my selection for suboptimal spectral counts.', Ui_MainWindow.spectralCounts)
    ...
    def onRandomForestClicked(self):
        ...
        Ui_MainWindow.RandomForest.sourceList.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)
        Ui_MainWindow.RandomForest.predictionList = QtWidgets.QListWidget()
        Ui_MainWindow.RandomForest.predictionList.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)
        Ui_MainWindow.RandomForest.predictionbtn = QtWidgets.QPushButton('->', self)
        ...
        for element in Ui_MainWindow.metrics.iloc[: ,0]:
             QtWidgets.QListWidgetItem(element, Ui_MainWindow.RandomForest.sourceList);
        ...
        hbox3.addStretch()
        Ui_MainWindow.RandomForest.happybtn = QtWidgets.QPushButton('I am happy with my selection', self)
        ...
        global items
        Ui_MainWindow.RandomForest.items = self.RandomForest.sourceList.selectedItems()
        Ui_MainWindow.RandomForest.sourceList.itemSelectionChanged.connect(self.sourceSelectionChanged)
        Ui_MainWindow.RandomForest.predictionbtn.clicked.connect(self.moveToPrediction)
    ...
    def moveToPrediction(self):
        ...
        for element in range(0,len(items)):
             QtWidgets.QListWidgetItem(items[element].text(), Ui_MainWindow.RandomForest.predictionList)
    ...
    def retranslateUi2(self):
        ...
        Ui_MainWindow.outlierlistLabel.setText("The following runs were identified as outliers: ")
        Ui_MainWindow.PCA.plotlabel.setText("Principal components analysis of quality metrics for outlier detection:")
        font = QtGui.QFont()
        font.setPointSize(18)
    ...
    def retranslateUi(self):
        _translate = QtCore.QCoreApplication.translate
        Ui_MainWindow.tab.Outliers.setText(_translate("MainWindow", "Detect Outliers"))
        Ui_MainWindow.tab.IndMetrics.setText(_translate("MainWindow", "Individual metrics"))
