...
class OutlierTab(QtWidgets.QTabWidget):
    ...
    def createTabWidget(self,now):
                # --------------------------------------Widgets-------------------------------------------
                        ...
                        OutlierTab.PCA.Checkbox.stateChanged.connect(
                            lambda x: globalVars.var.enable_slot()
                            if x else globalVars.var.disable_slot())
                        ...
                        OutlierTab.PCA.Redobox.stateChanged.connect(
                            lambda x: globalVars.var.enable_reanalysis())
                    ...
                    # --------------------------------------Layout-------------------------------------------
                        ...
                        globalVars.var.EnableAnalysisButtons()
                        ...
                        globalVars.var.pdf.setEnabled(True)
    ...
    def update_annot(event):
        ...
        samplenames = globalVars.var.database.FindRealSampleNames( globalVars.var.database.currentDataset.index)
        if(len(samplenames) != len(set(samplenames))):
            # if there are duplicates in the Main.filenames column like RTsegments
            # or per swath metrics
            sampleNameColumn1Combination = samplenames[chosensample] + "-" \
                + str(globalVars.var.database.currentDataset.iloc[chosensample, 1])
    ...
    def retranslateUi2(self):
        ...
        OutlierTab.outliercolorlabel1.setFont(globalVars.var.boldfont)
        OutlierTab.outliercolorlabel2.setFont(globalVars.var.boldfont)
        ...
        if(len(globalVars.var.outlierlist) > 0):
            outlierstring = ""
